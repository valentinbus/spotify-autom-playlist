...
class Spotify:
    ...
    def _get_baerer_token(self, code):
        ...
        self.baerer_token = content.get(
            'access_token')  # use for all api request
        return HOME_URL
            ...
            if result.json().get('genres'):
                ...
                for category_name in result.json().get('genres'):
                    if db.session.query(Category).filter_by(name=category_name).first() is None:
                        ...
                        category_id = db.session.query(Category).filter_by(
                    ...
                    if db.session.query(CategoryTrack).filter_by(track_id=track_id).first() is None:
                        category_track = CategoryTrack(
                            track_id=track_id, category_id=category_id)
    ...
    def _init_first_playlist(self, token, user_id, track_id):
        ...
        if db.session.query(Playlist).filter_by(user_id=user_id, name="Loved Tracks").first() is None:
            ...
            playlist = db.session.query(Playlist).filter_by(
                user_id=user_id, name='Loved Tracks').first()
    ...
    def _add_track(self, token, playlist_name, playlist_id, user_id):
        ...
        """
        category_id = db.session.query(Category).filter_by(
            name=playlist_name).first().id
        playlist_id_db = db.session.query(Playlist).filter_by(
            name=playlist_name, user_id=user_id).first().id

        loved_track_playlist = db.session.query(Playlist).filter_by(
            user_id=user_id, name="Loved Tracks").first()
        user_loved_track = db.session.query(TrackPlaylist).filter_by(
            playlist_id=loved_track_playlist.id)
        list_track_id = list()
        ...
        for loved_track in user_loved_track:
            ...
            if result:
                track_id = result[0]
                category_track = TrackPlaylist(
                    playlist_id=playlist_id_db, track_id=track_id)
                db.session.add(category_track)
                db.session.commit()
        ...
        i = 0
        # have to do this because if URI is too long request return 414
        while i <= len(list_track_id):
            headers = {
                'Authorization': token,
            }
    ...
    def clear_db(self, token, user_id):
        ...
        [TrackPlaylist.query.filter_by(
            playlist_id=playlist.id).delete() for playlist in playlists]
        playlists.delete()
    ...
    def get_tracks(self, user_id):
        ...
        playlist_id = db.session.query(
            Playlist).filter_by(user_id=user_id).first().id
        db.session.query(Track).filter_by(id=db.session.query(
            TrackPlaylist.track_id).filter_by(playlist_id=playlist_id))
    ...
    def suggest_playlist(self, user_id):
        """
        Purpose category playlist based on categories that come up
        the most in the liked songs
        """
        response = {
        ...
        playlist_id = db.session.query(Playlist).filter_by(
            name="Loved Tracks", user_id=user_id).first().id
        tracks_playlist = db.session.query(
            TrackPlaylist).filter_by(playlist_id=playlist_id)
        categories = list()
        for track_playlist in tracks_playlist:
            track_id = track_playlist.track_id
            try:
                category_id = db.session.query(CategoryTrack).filter_by(
                    track_id=track_id).first().category_id
        ...
        revelant_cat = sorted(
            category_counter, key=category_counter.get, reverse=True)
        top_10 = revelant_cat[:10]
        print(top_10)

        for category_id in top_10:
            query = db.session.query(Category).filter_by(
    ...
    def create_playlist(self, q, token, user_id):
        ...
        if q.isdigit() and db.session.query(Category).filter_by(id=q).first() is not None:
            playlist_name = db.session.query(
                Category).filter_by(id=q).first().name
